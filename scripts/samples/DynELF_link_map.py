...
def getLink_map(addr, bitSz):
	if bitSz == 32:
		return u32(leak(addr + 4, wordSz).ljust(4, '\0'))
	else:
		return u64(leak(addr + 8, wordSz).ljust(8, '\0'))
...
def lookup(pid, symbol):
	...
	link_map = getLink_map(moduleGot, bits)
	log.info("Link_map located at:................... " + hex(link_map))
