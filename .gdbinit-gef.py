...
def mips_registers():
    ...
    return ["$zero     ", "$at       ", "$v0       ", "$v1       ", "$a0       ", "$a1       ", "$a2       ", "$a3       ",
            ...
            "$t8       ", "$t9       ", "$k0       ", "$k1       ", "$s8       ", "$sr       ", "$bad      ", "$cause    ",
            "$pc       ", "$sp       ", "$hi       ", "$lo       ", "$fir      ", "$fsr      ", "$ra       ", "$gp       ", ]
    ...
    def dereference_from(addr):
        ...
        while True:
            ...
            if addr is None:
                ...
                value_from_stack = long(DereferenceCommand.dereference(value))
                msg.append( format_address(value_from_stack) )
                examine_string = gdb.execute("x/s %#x" % value_from_stack, to_string=True).strip().replace("\n", "").replace("\r", "")
                stack_string = re.findall('"(.*)"', examine_string)
                if stack_string:
                    if '\\' not in stack_string[0]:
                        msg.append(stack_string[0])
                break
